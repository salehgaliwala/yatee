...
class WCMp_Frontend {
    ...
    public function woocommerce_shipping_package_name($package_name, $vendor_id, $package) {
        if ($vendor_id && $vendor_id != 0) {
            ...
            if ($vendor) {
                return $vendor->page_title . __('-Livraison', 'dc-woocommerce-multi-vendor');
            }
            return $package_name;
        }
        return $package_name;
    ...
    public function product_archive_vendor_info() {
        ...
        if (wcmp_is_store_page()) {
            ...
            if( $vendor ){
               // $image = $vendor->get_image() ? $vendor->get_image() : $WCMp->plugin_url . 'assets/images/WP-ssstdavatar.png';
